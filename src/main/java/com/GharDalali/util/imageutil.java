...
public class imageutil {
	...
	public String getImageNameFromPart(Part part) {
		...
		if (i > 0) {
			extension = imageName.substring(i);
			imageName = imageName.substring(0, i);
		}

		// Return the unique filename
		return imageName + "_" + UUID.randomUUID().toString().substring(0, 8) + extension;
	}
	 ...
	 *         otherwise.
	 */
	public String uploadImage(Part part, ServletContext context, String saveFolder) {
		...
		try {
			...
			return "/images/" + saveFolder + "/" + imageName;
		} catch (IOException e) {
	...
	public String getSavePath(ServletContext context, String saveFolder) {
		...
		String realPath = context.getRealPath("/images/" + saveFolder);
		return realPath;
	}
}